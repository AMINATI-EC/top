...
def start_server():
    ...
    # admin.htmlが存在するか確認
    if not (current_dir / "admin.html").exists():
        print("❌ エラー: admin.htmlが見つかりません！")
        print(f"📁 現在のディレクトリ: {current_dir}")
        print("\n以下を確認してください：")
        print("1. このスクリプトをプロジェクトのルートフォルダに配置")
        print("2. admin.htmlが同じフォルダに存在すること")
        input("\nEnterキーを押して終了...")
        sys.exit(1)
    ...
    try:
        with socketserver.TCPServer((HOST, PORT), Handler) as httpd:
            ...
            print("=" * 50)
            print("📝 アクセス可能なページ:")
            print(f"  • トップページ: http://{HOST}:{PORT}/admin.html")
            print(f"  • 管理画面: http://{HOST}:{PORT}/admin.html")
            print("=" * 50)
...
def open_browser():
    ...
    url = f"http://{HOST}:{PORT}/admin.html"
